...
- **高可靠投递与容错机制 (Reliable Delivery & Idempotency)**  
  请求默认留存 7 天、SQLite 单一持久化路径、租约投递 + 令牌确认、服务端长轮询唤醒（落库即达），并以幂等键去重。
...
- **v2/v3 线协议（唯一世代）**
  canonical JSON 签名、lease/ack、per-mode ACL 与可选 key-id 轮换；v1 世代已于 2026-09-19 移除，规范见 `docs/PROTOCOL-V2.md`。
...
| `lib/` | dsh 插件核心：提供五个 `agent_relay_*` 模型工具、v2/v3 客户端、workspace 租约隔离与侧边栏状态面板 |
