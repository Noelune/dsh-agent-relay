...
export class RelayClientV2 {
  ...
  async ask({ target, body, context, sessionRef = '', idempotencyKey, timeoutSeconds = 240, executionMode = 'read', waitOffline = false }) {
    ...
    // Nobody is listening: don't burn the deadline. The request is retained for
    // days regardless, so the caller gets an honest answer now and can re-ask or
    // poll for status later. `waitOffline: true` opts back into queue-and-wait.
    if (!result.target_online && !waitOffline) {
      return {
        ok: false,
        ...result,
        reason: 'peer_offline',
