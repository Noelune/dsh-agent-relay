...
export class RelayClient {
  ...
  async #request(method, path, body, { retries = 0 } = {}) {
    ...
    try {
      ...
      if (code === 'duplicate') return { accepted: true, id: body?.id, duplicate: true }
   ...
   * v1.1 optional fields: kind (message|request|reply), rootId, parentId.
   * @returns {Promise<{accepted: boolean, id: string, duplicate?: boolean}>}
   */
  async send({ to, body, type = 'message', replyTo = null, ack = false, id = randomUUID(), kind, rootId, parentId }) {
   ...
   */
  async recv(limit = 50) {
    const path = `/messages?since=${encodeURIComponent(this.since ?? '')}&limit=${encodeURIComponent(limit)}`
  ...
  /** Lease-based pull (v1.1): lease up to `limit` queued messages for this agent. */
  async pull(limit = 50, leaseSeconds) {
    const body = { limit }
    if (leaseSeconds) body.leaseSeconds = leaseSeconds
    const result = await this.#request('POST', '/v1/pull', body)
    return result ?? { messages: [], count: 0 }
  }

  /** Acknowledge a leased message with a v1.1 outcome (completed | retry). */
  async ackOutcome(messageId, leaseId, outcome, error) {
    const body = { messageId, leaseId, outcome }
    if (error) body.error = error
    return this.#request('POST', '/v1/ack', body)
  }

  /** Batch status lookup (v1.1). */
  async status(messageIds) {
    const result = await this.#request('POST', '/v1/status', { messageIds })
    return result?.messages ?? []
  }

  /** Recent messages addressed to this agent (read-only, v1.1). */
  async recent(limit = 50) {
    const result = await this.#request('POST', '/v1/recent', { limit })
    return result?.messages ?? []
  }

  /** Read-only filtered search (v1.1). */
  async query(filters = {}) {
    const result = await this.#request('POST', '/v1/messages/query', filters)
    return result?.messages ?? []
  }
