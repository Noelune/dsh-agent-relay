...
export function apply(ctx, config = {}) {
  ...
  function notConfigured(action) {
    return {
      ok: false,
      error: `dsh-agent-relay not configured: set DSH_RELAY_SECRET or configure secretEnv/secretRef (${action}). Deploy the broker first — read docs/AGENT-DEPLOY.md and follow it end-to-end.`,
    }
  }
  ...
  async function resolveSecret() {
    ...
    const vaultPath = join(process.env.LOCALAPPDATA || '', 'hermes', 'scripts', 'secret_vault.py')
    const subprocess = ctx.get('subprocess')
    if (!subprocess || !existsSync(vaultPath)) return ''
    let pythonPath = 'python'
    try { pythonPath = await subprocess.resolveExecutable('python') } catch { /* fixed fallback */ }
    let stdout = ''
    try {
      const handle = subprocess.spawn({
        argv: [pythonPath, '-c', VAULT_REVEAL_SCRIPT, vaultPath, cfg.secretRef],
  ...
  // ---- broker lifecycle (follows dsh — no OS-level service) ------------------
  ...
  const TOOL_DEFS = [
    {
      ...
      description: '向本机 agent-relay 协作圈内的其他 Agent（codex、claude、hermes、openclaw、dsh）发送协作请求（审查、问题核查、实现任务）。请求必须自包含：对方看不到你的对话，必要背景写进 message 或 context。可传 memory_query 让系统自动把 unified-agent-memory 共享记忆摘录挂进 context（需配置 UNIFIED_MEMORY_CMD）。',
      parameters: {
  ...
  const ready = (async () => {
    ...
    sweepInterval = setInterval(() => { try { sweepIdleSessions() } catch { /* best-effort */ } }, SWEEP_INTERVAL_MS)
    ...
    ensureBroker().finally(() => scheduleNextPoll())
  })().catch((error) => {
  ...
  async function statusPayload() {
    const now = new Date()
    if (!client) return buildRelayStatusSnapshot({ cfg: null, peers: [], messages: [], lastError: lastError || 'relay 未配置', connected: false, now })
    let peers = []
    let messages = []
    ...
    let error = lastError
    try {
      const result = await executePeers()
    ...
    try {
      const result = await executeHistory({ limit: 20 })
