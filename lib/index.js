...
function resolveConfig(config = {}) {
  ...
  return {
    ...
    // Long-poll hold against a 0.6+ broker (it caps the wait at 120 s). An older
    // broker ignores `wait_seconds` and the fast/idle timers above take over
    // exactly as before, so this is safe to leave on.
    longPollSeconds: num(config.longPollSeconds ?? env.DSH_RELAY_LONG_POLL_SECONDS ?? fileCfg.longPollSeconds ?? fileCfg.long_poll_seconds, 0, 120, 25),
