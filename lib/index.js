...
function resolveConfig(config = {}) {
  ...
  return {
    ...
    leaseSeconds: Number.isFinite(leaseSeconds) ? Math.min(86_400, Math.max(1, leaseSeconds)) : 600,
  }
}
...
function receiptFile(agent) {
  const suffix = createHash('sha256').update(agent).digest('hex').slice(0, 16)
  return join(homedir(), `.dsh-agent-relay-receipts-${suffix}.json`)
}

/** Load completed-message receipts for one agent (durability across restarts). */
...
/** Persist one agent's receipts atomically (tmp + rename). Best-effort. */
function saveReceipts(file, receipts) {
  try {
    const tmp = file + '.tmp'
...
export function apply(ctx, config = {}) {
  ...
  const leased = new Map()  // buffered message id -> leased message
  const history = []        // local send/receive log, in memory only
  const seen = new Set()    // processed delivery attempts: message id + lease id
  let lastError = null
  let lastPollAt = null
  ...
  const receiptsFile = receiptFile(cfg.agent)
  const receipts = loadReceipts(receiptsFile)
  ...
  async function pollOnce() {
    ...
    try {
      ...
      for (const msg of messages ?? []) {
        const deliveryKey = `${msg.id}:${msg.leaseId ?? ''}`
        ...
        if (msg.type === 'ack') {
          ...
          await client.ackOutcome(msg.id, msg.leaseId, 'completed')
          continue
        }
        ...
        const prior = leased.get(msg.id)
        if (prior?.leaseId !== msg.leaseId) {
          const priorIndex = inbox.findIndex((item) => item.id === msg.id)
          if (priorIndex !== -1) inbox.splice(priorIndex, 1)
        }
        inbox.push(msg)
        leased.set(msg.id, msg)
    ...
    async execute(args) {
      ...
      try {
        ...
        if (args.replyTo) {
          ...
          saveReceipts(receiptsFile, receipts)
          const replyToLease = leased.get(args.replyTo)?.leaseId
          if (replyToLease) {
            try { await client.ackOutcome(args.replyTo, replyToLease, 'completed') } catch { /* lease expires and the broker re-delivers later */ }
            leased.delete(args.replyTo)
          }
        }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      try {
        ...
        // Do NOT lease-complete messages here: completing on read would turn the
        // reliable-delivery guarantee into at-most-once (a request read but not
        // answered — e.g. the session crashes mid-turn — would be marked done and
        // never redelivered). Instead the lease is completed when the model
        // replies via relay_send(..., replyTo), and an expired lease re-delivers
        // the message on a later poll (deduplicated by the per-lease `seen` set).
        // For messages that explicitly request a delivery receipt, send a v1.0
        // ack receipt to the sender (best-effort) without touching the lease.
