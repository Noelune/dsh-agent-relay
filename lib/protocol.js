/**
 * v2/v3 wire protocol primitives (compatible with the self-use Python
 * `relay/protocol.py` at both protocol generations). Single canonical copy —
 * broker/src/protocol.js re-exports this file; edit here only.
 *
 * Canonical body:   JSON with sorted keys, compact separators, non-ASCII kept
 *                   raw UTF-8 (Python ensure_ascii=False).
 * Signature string: v2 (no key-id header) —
 *                   HMAC(secret, agent\n ts\n METHOD\n path\n sha256hex(body))
 *                   v3 (X-Agent-Relay-Key-Id present) —
 *                   HMAC(secret, agent\n keyId\n ts\n METHOD\n path\n sha256hex(body))
 * Headers:          X-Agent-Relay-Agent / -Key-Id / -Timestamp / -Signature
 */
...
// Retention must survive the recipient being offline: on a single-user machine
// the pollers come and go with the apps that own them. The old 3600 s default
// made silent expiry the normal outcome rather than the exception — the
// 2026-09-19 audit of relay-v2.db found 15 of 27 messages lost with attempts=0
// (the target never pulled once). Requests therefore live for 7 days by default,
// matching `broker.messageTtlDays`; `ttl_seconds` remains an optional override.
export const DEFAULT_REQUEST_TTL_SECONDS = 7 * 86400
export const TTL_MIN_SECONDS = 60
export const TTL_MAX_SECONDS = 30 * 86400
// Presence: an agent is considered reachable when it pulled within this window.
// Comfortably above the slowest client-side idle poll interval (15 s).
export const PRESENCE_STALE_SECONDS = 90
// Upper bound for a long-poll `wait_seconds` hold (see broker/src/server.js).
...
/** Build the v2/v3 signature (matches self-use make_signature). */
export function makeSignature(agent, secret, method, path, timestamp, body, keyId = '') {
  const digest = createHash('sha256').update(body).digest('hex')
  const parts = keyId
    ? [agent, keyId, timestamp, method.toUpperCase(), path, digest]
    : [agent, timestamp, method.toUpperCase(), path, digest]
  return createHmac('sha256', secret).update(parts.join('\n')).digest('hex')
}

/** Constant-time v2/v3 signature check (matches self-use verify_signature). */
export function verifySignature(agent, secret, method, path, timestamp, body, signature, keyId = '') {
  if (typeof signature !== 'string' || signature.length === 0) return false
  const expected = makeSignature(agent, secret, method, path, timestamp, body, keyId)
...
export class RelayMessage {
  constructor({
    ...
    allow_shared_write: allowSharedWrite = false,
    lease_token: leaseToken = '', lease_until: leaseUntil = null,
  }) {
    ...
    this.allow_shared_write = Boolean(allowSharedWrite)
    // Delivery credentials from a pull response (broker-generated, never sent
    // back by clients except in ack/renew calls).
    this.lease_token = String(leaseToken || '')
    this.lease_until = leaseUntil == null ? null : Number(leaseUntil)
  }
  ...
  static fromDict(payload) {
    return new RelayMessage({
      ...
      topic: payload.topic || '',
      allow_shared_write: payload.allow_shared_write || false,
      lease_token: payload.lease_token || '',
      lease_until: payload.lease_until ?? null,
    })
  }
  ...
  toDict() {
    ...
    if (this.allow_shared_write) result.allow_shared_write = true
    return result
  }
}
