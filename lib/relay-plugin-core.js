...
/** Normalize a pulled v2/v3 message into a plain object with defaulted fields. */
export function normalizeMessage(m) {
  return {
    ...
    // v3 delivery credential from the pull (empty on token-less brokers).
    lease_token: String(m?.lease_token || ''),
    lease_until: m?.lease_until == null ? null : Number(m.lease_until),
  }
}
...
export function extractReplyText(session, boundary) {
  let text = ''
  // DSH core 0.1.5-rc.2 removed the iterable `session.events`; prefer the
  // `snapshotEvents(fromSeq)` half-open accessor (fromSeq is boundary-inclusive,
  // same semantics as the seq guard below), fall back to the old array.
  let events = []
  if (typeof session?.snapshotEvents === 'function') {
    try {
      const snapshot = session.snapshotEvents(boundary)
