...
export function prepareRelayWorkspace({ baseWorkspace, worktreeParent, agentName, messageId, executionMode }) {
  ...
  // Canonicalize both sides (macOS TMPDIR is often a symlink that git resolves
  // to /private/...), so the base's position inside the repo is computed on
  // resolved paths and the worktree workspace lands where it actually exists.
  let rel = ''
  try {
    const baseReal = realpathSync(base)
    const repoReal = realpathSync(repoRoot)
    const candidate = relative(repoReal, baseReal)
    if (candidate !== '' && !candidate.startsWith('..') && !isAbsolute(candidate)) rel = candidate
  } catch { rel = '' }
